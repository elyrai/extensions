...
export function createMessageBox(data?: any) {
  ...
  newElem.style.cssText =
    "tab-size: 4; font-feature-settings: normal; font-variation-settings: normal; box-sizing: border-box; display: flex; flex-direction: column; width: 100%; align-items: center; justify-content: space-between; overflow: hidden; border-radius: 0.5rem; background-color: rgb(27 32 45); padding: 0.5rem; font-family: 'Inter', sans-serif; margin-top: 2rem;";
  newElem.id = "data-box";
  ...
  newElem.innerHTML = `
    ...
    <button 
      type="submit"
      style="padding: 1rem; border: none; border-radius: 0.75rem; background-color: hsla(83,81%,73%,.1) ;color: #c7f284; cursor: pointer; onhover: background-color: hsla(83,81%,73%,.2); width: 30%"
      onmouseover="this.style.border = '1px solid #c7f284';"
...
export function addFirstData(data: any) {
  ...
  newElem.innerHTML = `
        <div style="display: flex; gap: 1rem">
          <p style="color: white; margin: 0">24h volume:</p>
          <p style="color: white; margin: 0">${data.volume24h}$</p>
        </div>
        <div style="display: flex; gap: 1rem">
          <p style="color: white; margin: 0">Liquidiy</p>
          <p style="color: white; margin: 0">${data.liquidity}$</p>
        </div>
        <div style="display: flex; gap: 1rem">
          <p style="color: white; margin: 0">Holders:</p>
          <p style="color: white; margin: 0">${data.holders}</p>
        </div>
        <div style="display: flex; gap: 1rem">
          <p style="color: white; margin: 0">Top 10 holders supply:</p>
          <p style="color: white; margin: 0">${data.top10Holders}%</p>
        </div>
        <div style="display: flex; gap: 1rem">
          <p style="color: white; margin: 0">Rug check score:</p>
          <p style="color: white; margin: 0">${data.rugcheckScore} - ${
    data.risk
  }</p>
        </div>
        ${
          !data.risks || data.risks.length === 0
            ? `
        <div style="display: flex; gap: 1rem">
          <p style="color: white; margin: 0">Rug risks:</p>
        </div>
          `
            : `
        <div style="display: flex; gap: 1rem">
        ...
        <ul style="color: white; margin: 0; list-style-type: disc; padding: revert">
          ${data.risks
...
export function addMessage(message: any) {
  ...
  newElem.innerHTML = `
    <div style="display: flex; gap: 1rem">
      <p style="color: white; margin: 0">${message.message}</p>
    </div>
  `;
...
export function addLoadingBox() {
  ...
  const newElem = document.createElement("div");
  newElem.style.cssText =
    "tab-size: 4; font-feature-settings: normal; font-variation-settings: normal; box-sizing: border-box; display: flex; width: 100%; align-items: center; justify-content: space-between; overflow: hidden; border-radius: 0.5rem; background-color: rgb(27 32 45); padding: 0.5rem; font-family: 'Inter', sans-serif; margin-top: 2rem;";
