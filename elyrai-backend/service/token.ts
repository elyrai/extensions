...
import { formatNumber, formatNumberWithDots } from "../utils/formatNumber";
...
export class TokenProvider {
  ...
  async formatTokenReport(data: ProcessedTokenData): Promise<string> {
    let output = "";

    output += `CA: ${this.tokenAddress}\n`;
    if (data.dexScreenerData.pairs.length > 0) {
      ...
      output += `${data.dexScreenerData.pairs[0].chainId} @ ${data.dexScreenerData.pairs[0].dexId}`;
      if (
        data.dexScreenerData.pairs[0].boosts &&
        data.dexScreenerData.pairs[0].boosts.active > 0
      ) {
        output += `Boosts Active: ${data.dexScreenerData.pairs[0].boosts.active}`;
      }
      output += `\n`;
    }

    output += `USD: $${toBN(data.tradeData.price).toFixed(6)}\n`;
    output += `24h Volume: $${toBN(data.tradeData.volume_24h_usd).toFixed(
      2
    )}\n`;
    output += `Liquidity: $${toBN(
      data.dexScreenerData.pairs[0].liquidity.usd
    ).toFixed(2)}\n`;
    output += `1h Change: ${data.tradeData.price_change_1h_percent}%\n`;
    output += `Holders: ${data.tradeData.holder}\n`;
    output += `Top 10 Holders: ${data.security.top10HolderPercent}%\n`;
    output += `Red flag: ${
      (await this.shouldTradeToken(data)) ? "No" : "Yes"
    }\n`;
    output += `Rugcheck Score: ${data.rugcheckData.score}\n`;
  ...
  async getFormattedTweetReport(): Promise<string> {
    try {
